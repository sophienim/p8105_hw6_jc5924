...
homicides_summary=homicides %>% 
  ...
  mutate(city_state=str_c(city,state,sep = ", "),
         ...
         status=case_when(disposition=="Closed by arrest"~1,
...
#### glm
```{r}
baltimore_df=homicides_summary %>% 
...
cities_glm = 
  ...
  mutate(models = map(.x = data, ~ glm(resolved ~ victim_age + victim_race + victim_sex,family = binomial(),data = .x)),
         results = map(models, broom::tidy)) %>% 
  unnest(results) %>% 
  filter(term == "victim_sexMale") %>% 
  mutate(odds_ratio = exp(estimate),
